...
2. 排除所有已经学习过的古诗；待温习、待巩固或薄弱项目只进入练一练与巩固入口。
3. 结合近期喜欢的主题、意象和阅读画像。
4. 按难度递进，并对连续同类内容施加多样性约束。
5. `GET /recommend/today` 返回当前推荐；`exclude_poem_id` 可用于“换一首”。

推荐列表会在基础分排序后执行相邻多样性重排，尽量错开同主题和同作者诗歌。

### 图片候选与诗库解析

- 推荐列表、搜索结果和图片候选统一复用 `PoemCard` 核心字段；规范主键为 `poem_id`，同时保留值相同的 `id` 兼容旧前端。
- `POST /poems/candidates` 新端侧入参仅使用 `content_type=poem_text|scene`：文字类型读取嵌套 `poem`，风景类型读取外层 `objects`；后端仍兼容旧字段。
